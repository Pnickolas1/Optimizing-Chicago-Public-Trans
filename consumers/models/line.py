...
class Line:
    ...
    def _handle_arrival(self, message):
        ...
        if prev_dir is not None and prev_station_id is not None:
            ...
            if prev_station is not None:
                ...
                logger.debug(
                    "unable to handle previous station due to missing \
                    station"
                )
        else:
            logger.debug(
    ...
    def process_message(self, message):
        ...
        if topic == "org.chicago.cta.stations.table.v1":
        ...
        elif "org.chicago.cta.station.arrivals.v1" == topic:
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY" == topic:
            ...
            logger.debug(
                "unable to find handler for message from topic '%s'",
                message.topic
            )